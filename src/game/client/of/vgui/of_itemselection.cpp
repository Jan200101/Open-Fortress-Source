...
CTFItemSelection::CTFItemSelection(Panel *parent, const char *panelName, const int iID) : CTFEditableButton(parent, panelName)
{
	...
	pSkin = new vgui::Button(this, "StyleButton", "+", this, "ChangeStyle");
...
void CTFItemSelection::OnCommand(const char *command)
{
	if ( FStrEq( command, "ChangeStyle" ) )
	{
		iSkin++;
		if( iSkin >= iSkinCount )
		{
		...
		if( !pItemList || pItemList->pSelectedItem != this )
			SetSelected(true);
	}
}
