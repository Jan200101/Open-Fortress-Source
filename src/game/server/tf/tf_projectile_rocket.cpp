...
// Purpose:
//-----------------------------------------------------------------------------
LINK_ENTITY_TO_CLASS(tf_projectile_flakball, CTFProjectile_FlakBall);
PRECACHE_REGISTER(tf_projectile_flakball);

IMPLEMENT_NETWORKCLASS_ALIASED(TFProjectile_FlakBall, DT_TFProjectile_FlakBall)

BEGIN_NETWORK_TABLE(CTFProjectile_FlakBall, DT_TFProjectile_FlakBall)
END_NETWORK_TABLE()

#define FLAKBALL_SPEED 750.f
#define FLAKBALL_MODEL "models/weapons/w_models/w_grenade_bouncer.mdl"

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
CTFProjectile_FlakBall::CTFProjectile_FlakBall()
{
	szImpactHard[0] = '\0';
	szImpactSoft[0] = '\0';
}
...
// Purpose:
//-----------------------------------------------------------------------------
CTFProjectile_FlakBall *CTFProjectile_FlakBall::Create(CTFWeaponBase *pWeapon, const Vector &vecOrigin, const QAngle &vecAngles, CBaseEntity *pOwner, CBaseEntity *pScorer, float creationTime, Vector velocity)
{
	CTFProjectile_FlakBall *pRocket = static_cast<CTFProjectile_FlakBall*>(CTFBaseRocket::Create(pWeapon, "tf_projectile_flakball", vecOrigin, vecAngles, pOwner));
	if (!pRocket)
		return NULL;
	...
	pRocket->SetElasticity(0.9f);
		...
		if (angle.x < 270.f && angle.x > 180.f)
			angle.x = 270.f;
		AngleVectors(angle, &vel);
		pRocket->SetAbsVelocity(vel * FLAKBALL_SPEED);

		pRocket->SetThink(&CTFProjectile_FlakBall::FlyThink);
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFProjectile_FlakBall::Precache(void)
{
	PrecacheModel(FLAKBALL_MODEL);
	PrecacheParticleSystem("stickybombtrail_blue");
	PrecacheParticleSystem("stickybombtrail_red");
	PrecacheParticleSystem("stickybombtrail_dm");
	PrecacheScriptSound("BouncerGrenade.ImpactHard");
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFProjectile_FlakBall::Spawn(void)
{
	SetModel(FLAKBALL_MODEL);
	Vector angle = Vector(RandomFloat(-1.f, 1.f), RandomFloat(-1.f, 1.f), RandomFloat(-1.f, 1.f));
	VectorNormalize(angle);
	m_rotationVector = QAngle(angle.x, angle.y, angle.z) * 20.f;
	CTFBaseRocket::Spawn();
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFProjectile_FlakBall::FlyThink(void)
{
	if (gpGlobals->curtime > m_flCollideWithTeammatesTime && m_bCollideWithTeammates == false)
		m_bCollideWithTeammates = true;
